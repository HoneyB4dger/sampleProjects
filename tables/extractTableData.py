...
for pageToAnalyse in range (1, 231):
  #print pageToAnalyse
  ...
  def isItNumber(text):
      # TODO: More currencies
      text = text.replace(",", "").replace("$", "").replace(" ", "").replace("%", "").replace(u"\xA3", "").replace(
          "(", "").replace(")", "")
      try:
          a = float(text)
          return True
  ...
  try:
      for col in range(0, len(df[0]["data"][0]) - 1):
          ...
          if columnToMerge:
              for row in range(0, len(df[0]["data"])):
                  df[0]["data"][row][col + 1]["text"] = df[0]["data"][row][col]["text"] + df[0]["data"][row][col + 1][
                      "text"]
          ...
          for col in range(startColumn + 1, lastColumn):
                  ...
                  if " " not in cell["text"]:
                      for previous_row_it in range(r_it - 1, 0, -1):
                          if df[0]["data"][listOfNumericalRows[previous_row_it]][col]["left"] != 0.0:
                              listOfRightValues.append(
                                  df[0]["data"][listOfNumericalRows[previous_row_it]][col]["left"] +
                                  df[0]["data"][listOfNumericalRows[previous_row_it]][col]["width"])
                              listOfCells.append([df[0]["data"][listOfNumericalRows[previous_row_it]][col]["left"] +
                                                  df[0]["data"][listOfNumericalRows[previous_row_it]][col]["width"],
                                                  cell["text"], account])
                              break
                  else:
              ...
              if cell["text"] != "" and (cell["left"] != 0.0 and cell["width"] != 0.0):
                  ...
                  # If multi cell value - split
                      ...
                      firstChunkWidth = cell["height"] * (0.81 * len(re.findall("[\d\(\-\+\$]", chunks[0])) + 0.41 * (
                      chunks[0].count(".") + chunks[0].count(",")) + 0.2)
          ...
          if len(c) > 2:
              ...
              for a in range(0, len(listOfCells)):
                  if listOfCells[a][0] < (float(sum(c)) / len(c) + 4):
                      print str(pageToAnalyse) + "|" + columnLabel + "|" + listOfCells[a][2] + "|" + listOfCells[a][1]
